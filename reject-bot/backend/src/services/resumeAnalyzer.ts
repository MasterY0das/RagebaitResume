...
export async function analyzeResume(
  filePath: string, 
  roastIntensity: 'mild' | 'medium' | 'savage' = 'medium',
...
`;
    ...
    if (feedbackMatch) {
      ...
      if (feedbackStartIndex !== -1) {
        ...
        const points = feedbackLines
          ...
          .filter(point => point.length > 0 && !point.includes('Real Feedback') && !point.includes('Verdict'));
        ...
        if (feedbackPoints.length === 0) {
          ...
          // Remove duplicate feedback points
          feedbackPoints = [...new Set(fallbackPoints)];
        }
      }
    }
    ...
    if (constructiveMatch) {
      ...
      // Remove duplicate feedback points
      const uniquePoints = [...new Set(points)];
      constructiveFeedback = uniquePoints;
    } else {
      // Fallback to a more generic approach if the specific regex fails
      const lines = aiResponse.split('\n');
      ...
      if (constructiveStartIndex !== -1) {
        ...
        // Remove duplicate feedback points
        constructiveFeedback = [...new Set(fallbackPoints)];
      }
    }
